...
<div class="content-wrapper">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="tbl_po_item_add" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>PR Number</th>
                                ...
                                <tbody>
                                    @foreach ($get_pr_data as $item_pr)
                                        <tr>
                                          ...
                                          <td>
                                            <div class="btn-group">
                                                ...
                                                @elseif($item_pr->pr_status == 9)
                                                    <button type="button" class="btn bg-success" title="Add PO" data-toggle="modal" data-target="#modal_po_item_add" id="getItemPO" data-url="{{ route('show_modal_add_po_item_hr_ga',['id'=>$item_pr->pr_no_1])}}"><i class="fas fa-plus"></i> Add PO</button>&nbsp;
                                                    <button type="button" class="btn bg-info" title="Show Detail" data-toggle="modal" data-target="#modal_pr_show_hr_ga" id="getPR" data-url="{{ route('show_modal_pr_hr_ga',['id'=>$item_pr->pr_no_1])}}"><i class="far fa-eye"></i></button>&nbsp;
                                                {{-- @elseif ($item_pr->pr_status  == 4)
                                                ...
                                                @if ( $item_pr->pr_status  == 3)
                                                    ...
                                                    <button type="button" class="btn bg-success" title="Add PO" data-toggle="modal" data-target="#modal_po_add" id="getPO" data-url="{{ route('show_modal_create_po_hr_ga',['id'=>$item_pr->po_no])}}"><i class="fas fa-plus"></i> Submit PO</button>&nbsp;
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table id="tbl_po" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($get_po_data as $item_po)
                                        <tr>
                                          <td>{{ $loop->iteration }}.</td>
                                          <td>{{ $item_po->pr_no_1 }}</td>
                                          <td>
                                            @if ($item_po->po_no == "")
                                                <p style="color: gray">PO Empty</p>
                                            @else
                                          ...
                                          <td>
                                            @if ( $item_po->pr_status  == 5)
                                              Waiting Manager/HR-GA Approval
                                            @elseif ( $item_po->pr_status == 4)
                                              PO Item Added - Please Add Price
                                            @elseif ( $item_po->pr_status == 3)
                                              PR Approved - PO Submitting
                                            @elseif ( $item_po->pr_status == 2)
                                              PR Approved - PO Submitting
                                            @elseif ( $item_po->pr_status == 1)
                                              PR PO Completed
                                            @elseif ( $item_po->pr_status == 6)
                                              PR Rejected
                                            @elseif ( $item_po->pr_status == 7)
                                              PO Rejected
                                            @elseif ( $item_po->pr_status == 9)
                                              PR Approved - Please Add PO
                                            @endif
                                          </td>
                                          <td>
                                            <div class="btn-group">
                                                ...
                                                @if ( $item_po->pr_status  == 5)
                                                    ...
                                                    <button type="button" class="btn bg-info" title="Show Detail" data-toggle="modal" data-target="#modal_pr_show_hr_ga" id="getPR" data-url="{{ route('show_modal_pr_hr_ga',['id'=>$item_po->pr_no_1])}}"><i class="far fa-eye"></i></button>&nbsp;
                                                    <button class="btn bg-secondary toastrDefaultError" title="Print PR"><i class="fas fa-print"></i></i></button>&nbsp;
                                                
                                                @elseif($item_po->pr_status == 9)
                                                    <button type="button" class="btn bg-success" title="Add PO" data-toggle="modal" data-target="#modal_po_item_add" id="getItemPO" data-url="{{ route('show_modal_add_po_item_hr_ga',['id'=>$item_po->pr_no_1])}}"><i class="fas fa-plus"></i> Add PO</button>&nbsp;
                                                    <button type="button" class="btn bg-info" title="Show Detail" data-toggle="modal" data-target="#modal_pr_show_hr_ga" id="getPR" data-url="{{ route('show_modal_pr_hr_ga',['id'=>$item_po->pr_no_1])}}"><i class="far fa-eye"></i></button>&nbsp;
                                                @elseif ($item_po->pr_status  == 4)
                                                  
                                                    <button type="button" class="btn bg-success" title="Add Price" data-toggle="modal" data-target="#modal_price_add" id="getPrice" data-url="{{ route('show_modal_price_hr_ga',['id'=>$item_po->po_no])}}"><i class="fas fa-plus"></i> Price</button>&nbsp;
                                                    <button type="button" class="btn bg-info" title="Show Detail" data-toggle="modal" data-target="#modal_pr_show_hr_ga" id="getPR" data-url="{{ route('show_modal_pr_hr_ga',['id'=>$item_po->pr_no_1])}}"><i class="far fa-eye"></i></button>&nbsp;
                                                    <form action="{{ route('print_pr_hr_ga') }}" method="POST">
                                                      @csrf
                                                      <input type="hidden" name="txt_pr_no" id="txt_pr_no" value="{{ $item_po->pr_no_1 }}">
                                                      <button type="submit" id="print_pr" class="btn bg-secondary" title="Print PR"><i class="fas fa-print"></i></i></button>&nbsp;
                                                    </form>
                                                  
                                                @endif
                                                @if ( $item_po->pr_status  == 3)
                                                  {{-- <div class="col-7">
                                                    <button type="button" class="btn btn-block btn-outline-primary" title="Show Data" data-toggle="modal" data-target="#modal_po_show_hr_ga" id="showPO" data-url="{{ route('show_modal_po_hr_ga',['id'=>$item_po->po_no])}}"><i class="fas fa-eye">&nbsp;View Data</i></button>
                                                  </div>
                                                  <div class="col-8">
                                                    <button type="button" class="btn btn-block btn-outline-success" title="Add PO" data-toggle="modal" data-target="#modal_po_add" id="getPO" data-url="{{ route('show_modal_create_po_hr_ga',['id'=>$item_po->po_no])}}"><i class="fas fa-plus">&nbsp;Submit PO</i></button>
                                                  </div> --}}
                                                  
                                                    <button type="button" class="btn bg-success" title="Add PO" data-toggle="modal" data-target="#modal_po_add" id="getPO" data-url="{{ route('show_modal_create_po_hr_ga',['id'=>$item_po->po_no])}}"><i class="fas fa-plus"></i> Submit PO</button>&nbsp;
                                                    <button type="button" class="btn bg-info" title="Show Detail" data-toggle="modal" data-target="#modal_po_show_hr_ga_po" id="showPO" data-url="{{ route('show_modal_po_hr_ga',['id'=>$item_po->po_no])}}"><i class="far fa-eye"></i></button>&nbsp;
                                                    <form action="{{ route('print_pr_hr_ga') }}" method="POST">
                                                      @csrf
                                                      <input type="hidden" name="txt_pr_no" id="txt_pr_no" value="{{ $item_po->pr_no_1 }}">
                                                      <button type="submit" id="print_pr" class="btn bg-secondary" title="Print PR"><i class="fas fa-print"></i></i></button>&nbsp;
                                                ...
                                                @elseif ( $item_po->pr_status == 1)
                                                    ...
                                                    <form action="{{ route('print_po_hr_ga') }}" method="POST">
                                                      @csrf
                                                      <input type="hidden" name="txt_po_no" id="txt_po_no" value="{{ $item_po->po_no }}">
                                                      <button type="submit" class="btn btn-block btn-outline-success" title="Print PO"><i class="fas fa-print">&nbsp;Print PO</i></button>
                                                    </form>
    ...
    <!-- /.content -->
    {{-- End Content --}}
    {{-- Create Modal Item PO --}}
    <form action="{{ route('store_item_po_hr_ga') }}" method="POST" enctype="multipart/form-data" id="po" name="po">
        @csrf
          <div class="modal fade" id="modal_po_item_add">
            <div class="modal-dialog modal-xl">
              <div class="modal-content">
                <div class="overlay" id="modal-loader_5">
...
<script>
    // Datatables
    $(function () {
      $("#tbl_po_item_add").DataTable({
        "responsive": true, "lengthChange": false, "autoWidth": false,
      }).buttons().container().appendTo('#tbl_po_item_add_wrapper .col-md-6:eq(0)');
      $("#tbl_po").DataTable({
        "responsive": true, "lengthChange": false, "autoWidth": false,
...
<script>
  $(document).ready(function(){
      ...
      $(document).on('click', '#getItemPO', function(e){
  
          e.preventDefault();
  
          var url = $(this).data('url');
  
          $('#dynamic-content_5').html(''); // leave it blank before ajax call
          $('#modal-loader_5').show();      // load ajax loader
          ...
          .done(function(data){
              console.log(data);  
              $('#dynamic-content_5').html('');    
              $('#dynamic-content_5').html(data); // load response 
              $('#modal-loader_5').hide();        // hide ajax loader   
          })
          .fail(function(){
              $('#dynamic-content_5').html('<i class="glyphicon glyphicon-info-sign"></i> Something went wrong, Please try again...');
              $('#modal-loader_5').hide();
          });
