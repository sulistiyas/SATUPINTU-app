                ...
                <tbody>
                  @foreach ($data_pr as $item_report)
                    <tr>
                      ...
                      <td align="center"><a href="{{ route('print_pr_epurchase',['id'=>$item_report->pr_no]) }}">{{ $item_report->pr_no }} <i class="fas fa-file-download"></i></a> </td>
                      <td>{{ $item_report->pr_date }}</td>
                      <td align="center">
                        @if ($item_report->po_no == "")
                          <b style="color: gray;">Data Empty</b>
                        @else
                          <a href="{{ route('print_po_epurchase',['id'=>$item_report->po_no]) }}">{{ $item_report->po_no }} <i class="fas fa-file-download"></i></a>    
                        @endif
